...
### 70. Расширение модели User. Класс AbstractUser.
...
#### Класс AbstractUser.
Согласно [документации Django](https://docs.djangoproject.com/en/4.2/topics/auth/customizing/#extending-the-existing-user-model), мы можем расширить существующую модель User следующим образом (в файле users/models.py):
```python
# users/models.py
...
### 71. Разрешения и группы (Permissions & Groups).
...
Как раз об этих группах и разрешениях и пойдет речь на этом занятии. Подробная документация о них приведена на [соответствующей странице](https://docs.djangoproject.com/en/4.2/topics/auth/default/#permissions-and-authorization).
...
```
Этот список взят из [документации](https://docs.djangoproject.com/en/4.2/topics/auth/default/#permissions-and-authorization).
...
## X. OAuth 2.0 - социальная аутентификация.
### 72. Протокол OAuth 2.0. Установка пакета Python-Social-Auth.
На предыдущих занятиях мы с вами научились делать авторизацию и регистрацию пользователей с помощью стандартного, встроенного во фреймворк Django, способом: путем ввода логина/пароля или E-mail/пароля. Однако с развитием социальных сетей, таких как ВКонтакте, Одноклассники, GitHub, и с появлением внешних аккаунтов, вроде аккаунта Google, аккаунта Яндекс и т. п. появилась возможность выполнять аутентификацию пользователей через эти ресурсы. Такой способ получил название социальной аутентификации или авторизации через социальные сети. На ближайших занятиях мы с вами научимся подключать этот вариант авторизации к проекту на Django. А начнем с принципа работы социальной аутентификации.

В основе авторизации через социальные сети лежит протокол под названием:

OAuth 2.0 (Open Authorization)

Общий принцип его работы следующий.

![Пример авторизации через соц. сети](images/oauth_ex.png)

Сначала пользователь переходит на сервер, где расположен наш сайт (или любой другой, с авторизацией через соцсети). Нажимает на кнопку авторизоваться и сервер (клиент) отправляет GET-запрос на сервер соцсети для получения разрешения выполнения процедуры аутентификации пользователя. Далее, серверы начинают общаться друг с другом без участия клиента. Если разрешение на аутентификацию получено (это получение определенного кода), то, затем, выполняется GET-запрос (с полученным кодом разрешения) на получение токена доступа к учетной записи пользователя. Если токен приходит на сервер сайта, то далее, он используется для получения данных учетной записи, например, имени, фамилии, электронной почты. Причем, пользователь должен разрешить извлечение этих данных, иначе, сервер соцсети их не отправит. Если все три запроса были выполнены успешно, то протокол аутентификации OAuth 2.0 на этом завершается и на сервере сайта начинается процедура авторизации пользователя на основе полученных данных.

Конечно, это лишь краткое описание принципа работы протокола OAuth 2.0, но, так как мы не будем его реализовывать вручную, нам этого понимания будет вполне достаточно. Здесь только следует добавить, что детали реализации этого протокола могут меняться для каждой соцсети. Это относится к набору передаваемых и принимаемых параметров в GET-запросах. Но общий принцип остается неизменным.

Итак, чтобы внедрить на наш сайт механизм авторизации пользователей через социальные сети лучше всего воспользоваться уже готовой библиотекой. Для фреймворка Django их разработано несколько, но наиболее известные следующие:
- Django-Allauth
- Django-Social-Auth
- Python-Social-Auth (развитие Django-Social-Auth)

Я выберу последнюю, так как она поддерживает довольно большое разнообразие социальных сетей и других сторонних сервисов для аутентификации пользователей. Подробная информация об этом пакете приведена на [странице официальной документации](https://python-social-auth.readthedocs.io/en/latest/index.html). Если открыть [раздел](https://docs.allauth.org/en/latest/socialaccount/providers/index.html), то мы увидим множество провайдеров (серверов аутентификации), которые поддерживает библиотека Python-Social-Auth. Здесь приведены все наиболее известные ресурсы, включая, GitHub (которым мы скоро воспользуемся), Одноклассники, ВКонтакте, Яндекс и так далее.

В разделе [конфигурации библиотеки под фреймворк Django](https://python-social-auth.readthedocs.io/en/latest/configuration/django.html) приведена подробная инструкция, как устанавливать и настраивать эту библиотеку в проекте сайта. Мы ей сейчас воспользуемся и установим пакет Python-Social-Auth.
#### Установка Python-Social-Auth.
Первым делом, согласно документации, необходимо скачать и установить библиотеку social-auth-app-django для текущего интерпретатора языка Python. В моем случае – это виртуальное окружение. Для установки я перейду в PyCharm, открою раздел File -> Settings и в появившемся окне, нажму на плюсик. Появится еще одно окно для установки пакетов в текущее виртуальное окружение, и уже здесь в окне поиска введу social-auth-app-django. Для выбранного пакета нажимаем кнопку «Install Package».

Все, библиотека скачана и установлена. Следующим шагом ее нужно подключить к нашему проекту. Опять же, согласно документации, переходим в файл settings.py и в коллекции INSTALLED_APPS прописываем этот пакет:
```python
# sitewomen/settings.py
INSTALLED_APPS = [
    ...
...
```
Далее, документация говорит, что нам нужно выполнить миграции и создать необходимые таблицы в БД. Сделаем это с помощью команды:
```shell
python3 manage.py migrate
```
Миграции должны быть успешно применены.

Следующим шагом необходимо в файле settings.py в коллекцию AUTHENTICATION_BACKENDS добавить бэкенд аутентификации пользователя по нужным соцсетям. Первым таким сервисом будет GitHub, поэтому я здесь пропишу строчку:
```python
# sitewomen/settings.py
AUTHENTICATION_BACKENDS = [
...
```
Причем, стандартные бэкенды мы тоже оставляем.

Наконец, последнее, что нужно сделать – это в файле sitewomen/urls.py добавить маршруты для установленного пакета:
```python
# sitewomen/urls.py
urlpatterns = [
    ...
...
```
Все, библиотека установлена и готова к работе. На следующем занятии мы настроим ее для авторизации пользователей через сервис GitHub.
### 73. GitHub-аутентификация.
Продолжаем тему социальной аутентификации и на этом занятии мы с вами реализуем авторизацию пользователей через сервис GitHub, используя ранее установленный пакет Python-Social-Auth.

Чтобы мы могли взаимодействовать с сервисом GitHub по протоколу OAuth 2.0 для аутентификации, в нем сначала нужно создать, так называемое приложение, перейдя по [ссылке](https://github.com/settings/applications/new).

Под приложением здесь подразумевается сторонний ресурс, который будет взаимодействовать с GitHub и аутентифицировать пользователя. Давайте введем здесь следующие данные:
- Application name: sitewomen
- Homepage URL: http://127.0.0.1:8000/
- Authorization callback URL: http://127.0.0.1:8000/social-auth/complete/github/

Обратите внимание, что GitHub позволяет нам указывать локальный хост в виде 127.0.0.1 и обращаться по протоколу http. Далеко не каждый сервис разрешает такие вольности. Некоторые соцсети требуют полноценный домен сайта и протокол взаимодействия https. Именно поэтому вначале я выбрал GitHub для демонстрации социальной авторизации пользователей. Также у вас может возникнуть вопрос, почему в callback URL определен именно такой маршрут? Это, конечно, было взято из документации пакета Python-Social-Auth на [странице настройки бэкенда GitHub для аутентификации](https://python-social-auth.readthedocs.io/en/latest/backends/github.html#github-apps).

После заполнения обязательных полей нажимаем на кнопку «Register application» и попадаем на страницу приложения. Здесь у нас есть Client ID – это ключ приложения, и Client secret – это закрытый (секретный) ключ приложения. Эти ключи необходимо знать, чтобы наш сервер мог взаимодействовать с GitHub по протоколу OAuth 2.0 для аутентификации пользователя. Собственно, это главная цель регистрации приложения.

Далее, согласно документации мы должны в файле settings.py прописать два параметра:
```python
# sitewomen/settings.py
SOCIAL_AUTH_GITHUB_KEY = 'xxxxx'
SOCIAL_AUTH_GITHUB_SECRET = 'xxxxxxxxxxx'
```
с открытым и секретным (закрытым) ключами созданного приложения.

Все, это минимально необходимые настройки для использования GitHub в процедуре аутентификации.

Следующим шагом мы добавим кнопку авторизации через GitHub в форме ввода логина и пароля:
```http request
http://127.0.0.1:8000/users/login/
```
Для этого в шаблоне users/login.html после формы пропишем строчки:
```html
<!-- logi.html -->
<hr>
<p><a href="{% url 'social:begin' 'github' %}"><img src="/media/social-auth/github.png"></a></p>
```
Опять же, информация о том, как прописать URL-адрес для авторизации с помощью GitHub (или любого другого бэкенда) приведена на странице [документации библиотеки Python-Social-Auth](https://python-social-auth.readthedocs.io/en/latest/use_cases.html#signup-by-oauth-access-token).

Все, кнопка авторизации размещена, переходим на страницу:
```http request
http://127.0.0.1:8000/users/login/
```
и нажимаем на GitHub. Если все было сделано правильно, то появится окно с разрешением доступа к аккаунту пользователя и перенаправление на наш сайт с уже авторизованным пользователем. Мы можем перейти в его профиль, который был создан ранее для расширенной модели пользователя, а также видим в таблице users еще одну запись с новым пользователем, вошедшим через GitHub. Как видите, все достаточно просто и удобно.
#### Добавление пользователей в отдельную группу.
Однако если мы внимательно посмотрим профиль пользователя, вошедшего через соцсеть, то увидим пункт «Изменить пароль». Очевидно, это лишнее, так как при социальной аутентификации используется токен, а затем, автоматически генерируется пароль. Вводить его или менять в системе уже не нужно. Поэтому было бы логично для таких пользователей пункт «Изменить пароль» не отображать. Но как это сделать? Вероятно, способов можно придумать множество. Я воспользуюсь возможностями фреймворка Django и пакета Python-Social-Auth. И первым делом перейдем в админ-панель Django. Здесь для пользователей мы можем создавать группы. И пусть те, что авторизовываются через соцсети попадают в группу social, которую мы с вами создадим и в эту группу добавим ранее созданное разрешение с именем Social Auth и кодовым именем social_auth. Следующим шагом нам нужно помещать пользователей в созданную группу. Для этого мы воспользуемся [механизмом под названием pipeline (конвейер)](https://python-social-auth.readthedocs.io/en/latest/pipeline.html).

В частности, в документации сказано, что мы можем в файле settings.py определить свой модуль pipeline и прописать его в специальной коллекции:
```python
# sitewomen/settings.py
...
```
Обратите внимание, что все другие модули в SOCIAL_AUTH_PIPELINE должны быть указаны, иначе наш пользовательский модуль может работать некорректно.

Осталось создать функцию new_users_handler() в файле users/pipeline.py следующим образом:
```python
# users/pipeline.py
from django.contrib.auth.models import Group
...
```
Все, теперь мы можем по группе идентифицировать пользователей, вошедших через социальные сети. Для этого в файле шаблона users/profile.html будем отображать ссылку «Сменить пароль» по следующему условию:
```html
<!-- profile.html -->
...
{% endif %}
```
Все, теперь профиль выглядит куда лучше. Хотя мы все равно можем сменить пароль, если вручную укажем ссылку:
```http request
http://127.0.0.1:8000/users/password-change/
```
Правда, тогда еще нужно как-то ввести прежний, старый пароль, который нам неизвестен. Поэтому как-то дополнительно защищать (закрывать) этот функционал не имеет особого смысла.
### 74. Добавляем поддержку https протокола и домен sitewomen.ru.
Сейчас у нас используется обычный не защищенный протокол http:
```http request
http://127.0.0.1:8000
```
Однако некоторые сервисы аутентификации требуют полноценный домен и https протокол. Давайте, это поправим. Первым шагом добавим возможность отображать сайт по доменному имени:
```http request
sitewomen.ru
```
В ОС Windows для этого достаточно открыть на редактирование специальный файл hosts, расположенный по маршруту:

C:\Windows\System32\drivers\etc\hosts

(если вы работаете под ОС Linux или macOS, то файл находится по адресу /etc/hosts) и добавить в него строчку:
```text
127.0.0.1       sitewomen.ru
```
После этого сохранить. Обратите внимание, что для этого у вас должны быть права администратора и антивирус не должен блокировать этот файл от изменений. Если антивирус мешает, то отключите его, поменяйте файл, и снова включите.

Проверить корректность работы этого изменения можно с помощью команды:
```shell
ping sitewomen.ru
```
Если вы увидите строчки:
```shell
Reply from 127.0.0.1: bytes=32 time…
```
то, значит, все работает как надо.

Следующим шагом нам нужно поместить домен sitewomen.ru в список разрешенных хостов. Открываем файл settings.py и в список ALLOWED_HOSTS добавляем следующие строчки:
```python
# sitewomen/settings.py
...
Если теперь перейти по адресу:
```http request
http://sitewomen.ru:8000
```
то увидим главную страницу нашего сайта. При этом все маршруты автоматически будут использовать этот домен.
#### Запуск сайта с протоколом HTTPS.
К сожалению, встроенный во фреймворк Django тестовый веб-сервер, не может раздавать сайт по HTTPS протоколу. Поэтому нам придется для этого использовать дополнительные инструменты. Я воспользуюсь расширением RunServerPlus из пакета Django Extensions. Причем, мы будем его здесь использовать исключительно в целях тестирования и отладки. В боевом режиме протокол HTTPS обеспечивается реальным веб-сервером.

Первым делом мы установим пакет Django Extensions, если он еще не установлен. В PyCharm я открою окно File->Settings, нажму «плюсик» и в строке поиска введу django-extensions. Выберу пакет с одноименным названием и нажму на кнопку «Install Package». Дополнительно здесь же установим еще один пакет Werkzeug, который содержит отладчик для расширения RunServerPlus. И последнее, что нам понадобится – это библиотека pyOpenSSL, которая отвечает за слой SSL/TSL протокола HTTPS.

Чтобы подключить приложение Django Extensions в файле settings.py в коллекции INSTALLED_APPS должна присутствовать строчка:
```python
    'django_extensions',
```
Все, теперь у нас должна быть доступна следующая команда для запуска сайта по протоколу HTTPS:
```shell
python3 manage.py runserver_plus --cert-file cert.crt
```
После этого открываем сайт по адресу (возможно, должна быть включена сеть Интернет):
```http request
https://sitewomen.ru:8000
```
и в браузере отображается предупреждение о недействительном сертификате. На это можно не обращать внимание, т.к. сертификат был сгенерирован автоматически с целью отладочного запуска сайта по протоколу HTTPS.
#### Добавление конфигурации запуска по HTTPS.
Чтобы в PyCharm нам постоянно не писать команду:
```shell
python3 manage.py runserver_plus --cert-file cert.crt
```
я создам еще одну конфигурацию для запуска веб-сервера. Для этого откроем окно Run/Debug Configurations и нажмем на плюсик. Дадим название «https sitewomen» новой конфигурации, все остальные параметры скопируем из предыдущей конфигурации, кроме поля parameters, где пропишем:
...
```
## XI. PostgreSQL и другие улучшения.
### 76. Работа с Django Fixtures.
До сих пор мы с вами при разработке сайта пользовались довольно простой СУБД SQLite. В реальных проектах веб-серверы взаимодействуют, как правило, с более продуктивными СУБД, такими как MySQL, PostgreSQL, Oracle и некоторыми другими. Однако прежде чем переходить к другим типам БД нам необходимо научиться быстро переносить данные из одной БД в другую. И, как вы уже догадались, фреймворк Django поддерживает этот функционал. Он связан, с так называемыми фикстурами (fixtures) и реализуется двумя основными командами:
```shell
python3 manage.py dumpdata <app_name>.<ModelName> -o <path_to_file.json>
python3 manage.py loaddata <path_to_fixture_file>
```
Давайте посмотрим, как ими можно воспользоваться для сохранения и последующего переноса данных.

Перейдем в приложение women и откроем файл models.py. В этом файле перечислены все наши модели. Нам нужно сохранить их данные в файлы фикстур. Я буду идти по порядку. Вначале мы видим модель Women. Следовательно, для формирования первого файла фикстуры нужно записать команду:
```shell
python3 manage.py dumpdata --indent=2 women.Women -o women/fixtures/women_women.json
```
Если выходной файл db.json сохраняется не в кодировке UTF8, то у интерпретатора Python можно дополнительно прописать флаг -Xutf8 следующим образом:
```shell
python3 -Xutf8 manage.py dumpdata --indent=2 --exclude contenttypes --exclude social_django -o db.json
```
Все json-файл со всеми нужными нам данными создан и может быть использован для загрузки в любые БД с соответствующим набором таблиц. То есть, таблицы, конечно же, должны иметь ту же структуру, что и данные в загружаемых json-файлах.

Давайте посмотрим, как можно выполнить эту загрузку. Я удалю из проекта файл БД и файлы миграций, чтобы имитировать пустой проект (без данных). Затем, создам заново файлы миграций:
```shell
python3 manage.py makemigrations
```
и применю их:
...
python3 manage.py migrate
```
Миграции были выполнены и таблицы созданы заново. Осталось загрузить в них прежнюю информацию по публикациям из файла db.json. Для этого выполним следующую команду:
```shell
python3 manage.py loaddata db.json
```
Посмотрим, что у нас получилось. Запустим веб-сервер, перейдем на сайт и видим прежнее содержимое вместе зарегистрированными пользователями.

Вот такой достаточно удобный инструмент имеется во фреймворке Django для переноса данных из одной СУБД в другую. Мы им воспользуемся на следующем занятии, когда будем переходить на новый тип БД PostgreSQL.
### 77. Подключаем PostgreSQL к проекту.
На этом занятии мы с вами установим, настроим и подключим СУБД PostgreSQL к нашему проекту. До сих пор использовалась БД по умолчанию типа SQLite. Она хорошо подходит для локальных приложений и учебных целей, так как проста в обращении и не требует отдельной установки. Но имеет серьезные ограничения по скорости работы с большими и сложными запросами, а также по предоставляемому функционалу. Поэтому в продакшене, конечно же, [СУБД](https://docs.djangoproject.com/en/4.2/ref/databases/) нужно менять и довольно часто проекты на Django связывают с PostgreSQL. Я также считаю это хорошим выбором.

Чтобы подключить PostgreSQL к нашему проекту, необходимо выполнить следующие шаги:
- Установить СУБД PostgreSQL на компьютер.
- Создать для этой СУБД базу данных и пользователя-администратора.
- Установить пакет psycopg2, связывающий PostgreSQL с Django.
- Подключить к проекту СУБД PostgreSQL.
- Выполнить миграции для создания необходимых таблиц.
- Загрузить в таблицы данные из ранее сохраненных фикстур.

Начнем с установки. Для этого следует перейти на [официальный сайт PostgreSQL](https://www.postgresql.org/) и нажать на ссылку «Download». Выбираем ОС Windows (или ту, на которой вы разрабатываете проект). Затем, по ссылке «Download the installer» скачиваем установщик этой СУБД версии 13.2 (у меня последняя версия 16 не заработала). Запускаем установщик, вводим пароль (1234), порт оставляем по умолчанию 5432.

После установки запускаем приложение pgAdmin, подключаемся к серверу PostgreSQL 13 (вводим пароль 1234) и попадаем на Dashboard. Следующим шагом создаем нового пользователя БД (Login/Group Roles) с именем sitewomen, паролем 1234 и всеми правами (привилегиями). Нажимаем «Save». Затем выбираем пункт Databases и в выпадающем меню «Create...». Создаем новую БД с именем sitewomen_db, владелец sitewomen и жмем кнопку «Save».

Все, пользователь и БД созданы, теперь нам нужно связать эту СУБД с нашим проектом на Django. Для этого в PyCharm для текущего интерпретатора установим пакет psycopg2. Для этого переходим по File -> Settings..., жмем «плюсик» и вводим название пакета psycopg2 и устанавливаем его. Затем, в файле settings.py меняем параметр DATABASES следующим образом:
```python
# sitewomen/settings.py
DATABASES = {
    'default': {
...
```
Для новой подключенной БД выполним миграции (для создания таблиц):
```shell
python3 manage.py migrate
```
и заново создадим суперпользователя:
```shell
python3 manage.py createsuperuser
```
Запустим наш сайт. Видим, что он сейчас совершенно пустой, так как таблицы сейчас не содержат никаких данных. Мы в этом можем убедиться, если зайти в админ-панель и просмотреть таблицы.

Давайте заполним эти таблицы прежним содержимым. Для этого в терминале выполним команду по загрузке ранее созданной фикстуры с данными прежней СУБД:
```shell
python3 manage.py loaddata db.json
```
...
### 80. Установка Redis для кэширования данных.
Когда мы с вами устанавливали Django Debug Toolbar, то видели сколько и какие SQL-запросы выполняются для формирования HTML-страницы. Здесь важно понимать, что эта операция повторяется для каждого клиентского запроса. И представьте, что ваш сайт посещают тысячи человек в сутки, значит, каждый час таких запросов может приходить сотни. А это еще не самая большая посещаемость. Обычным делом сейчас для сайтов является нагрузка в несколько сот тысяч, а то и миллионы запросов в сутки. Поэтому, те 3-5 SQL-запросов, что мы видим в Debug Toolbar можно смело умножать на тысячи, если имеем дело с высоконагруженным сайтом.

В таких ситуациях вполне серьезно встает вопрос о снижении нагрузки на сервер. Конечно, первым делом нужно оптимизировать SQL-запросы, чтобы они выполнялись как можно быстрее и снижали нагрузку на СУБД и сервер. Но существует и еще один довольно распространенный механизм, связанный с кэшированием страниц сайта или его фрагментов. Идея, в общем-то, очевидная. Если мы строим какой-либо информационный сайт, то содержимое его страниц обновляется не часто (включая дизайн). Поэтому при многочисленных запросах к одной и той же такой странице, было бы логично первый раз сформировать ее, а при повторных обращениях, выдавать уже сформированный HTML-документ. Это позволит заметно снизить нагрузку на СУБД и сервер. Давайте посмотрим, как это делается во фреймворке Django.

Если открыть документацию, то внизу страницы в разделе [«Common web application tools»](https://docs.djangoproject.com/en/4.2/#common-web-application-tools) можно увидеть список различных инструментов и первая ссылка ведет нас на [описание способов кэширования](https://docs.djangoproject.com/en/4.2/topics/cache/).

Здесь приведено несколько встроенных бэкендов для кэширования информации. Видим, что кэш в Django можно реализовать на уровне памяти (Memcached) – это самый быстрый тип кэша; с использованием БД Redis – наиболее распространенный способ (бэкенд добавлен в Django 4); и некоторые другие варианты. В чем же популярность использования хранилища Redis в качестве кэша? Смотрите, в своей основе процесс кэширования реализован на базе, так называемых, хэш-таблиц. В качестве ключей у них фигурируют URL-запросы пользователей, а в качестве значений – возвращаемое содержимое, например, сформированная HTML-страница. Особенностью хэш-таблиц является очень быстрый процесс поиска ключа (URL-запроса) и выдача данных. Среди всех известных структур данных, хэш-таблица делает операции записи и возврата значений по ключам самым быстрым способом. Именно поэтому словари и множества в Python реализованы на базе хэш-таблиц. А также и кэширование информации на серверах. Конечно, нам бы хотелось реализовывать кэш на уровне памяти с использованием Memcached бэкенда Django. Тем более, что согласно документации, он положен в основу таких популярных и высоконагруженных сервисов, как LiveJournal и Wikipedia. Но у него есть очевидный минус: для полноценной работы требуется большой объем оперативной памяти сервера. А это не дешевый ресурс. Поэтому пока мы не вышли на уровень LiveJournal или Wikipedia лучше воспользоваться компромиссным вариантом – хранилищем Redis. Это, своего рода, БД, но не реляционная, как PostgreSQL с набором таблиц и связей между ними, а, по сути, хэш-таблица. Redis позволяет записывать и получать данные в формате ключ-значение. Причем, пока позволяет определенный лимит памяти, данные заносятся в нее. Как только установленный уровень памяти превышается, наименее востребованные данные перемещаются в файл. Таким образом, Redis старается максимизировать скорость обращения к данным и в то же время способна работать с сильными ограничениями по памяти. Поэтому для начальных проектов – это хороший выбор хранилища для кэширования данных.
#### Установка и настройка Redis.
Давайте, перейдем непосредственно к установке и настройки механизма кэширования на базе Redis. В документации написано, что для этого нужно вначале установить саму БД Redis. Перейдем на [официальный сайт](https://redis.io/) И нажмем по ссылке «Get Started». Я выберу установку для ОС Windows (вы, соответственно, выбираете установщик для своей ОС). И здесь подробная инструкция, как установить БД Redis.

Устанавливаем WSL2. Открываем командное окно и выполняем команду: wsl --install. Вводим логин: serg и пароль: 1234.

Выполняем команды из раздела «Install Redis».

Запускаем сервер Redis: sudo service redis-server start (останавливается с помощью команды sudo service redis-server stop).

После этого нам нужно в PyCharm для текущего интерпретатора установить пакет redis-py с помощью команды:
```shell
pip3 install redis
```
и еще (для ускорения процесса работы Redis) рекомендуется поставить пакет hiredis:
```shell
pip3 install hiredis
```
После этого, в файле settings.py прописать специальный словарь CACHES следующим образом:
```python
# sitewomen/settings.py
CACHES = {
    "default": {
...
```
Все, механизм кэширования подключен к нашему проекту. Но пока он еще не используется, так как мы не указали, что именно и как собираемся кэшировать. Это мы рассмотрим на следующем занятии.
#### Области применения Redis.
В заключение отмечу, что БД Redis хорошо подходит для ряда задача при построении сайтов. Ниже перечислены области, в которых Redis может быть более лучшим выбором, чем классическая реляционная БД.

- Redis довольно удобная при хранении подсчете числа просмотров страниц сайта.
- С помощью Redis достаточно просто имитировать очереди типов LIFO и FIFO с данными. Для этого имеются команды lpush()/rpush() и lpop()/rpop().
- Кэширование, которое было использовано, как пример использования Redis.
- Отправка отложенных сообщений с использованием Redis.
- Быстрое создание рейтингов на основе упорядоченных множеств, поддерживаемых Redis.
- Логирование и отслеживание действий в реальном режиме времени также целесообразно делать на базе Redis.

Это лишь некоторые направления, где Redis может заметно упростить разработку программного кода и сделать его достаточно быстрым. Вообще, везде, где может понадобиться простой и быстрый доступ к данным по ключам, можно посмотреть в сторону этой быстродействующей БД.
### 80. Кэширование данных сайта.
Давайте, теперь посмотрим, как именно выполняется кэширование данных на сайте. Если обратиться к [документации](https://docs.djangoproject.com/en/4.2/topics/cache/#the-per-site-cache), то мы видим список различных параметров кэша, то есть, настраивать кэш и для страниц сайтов целиком и отдельно для шаблонов и устанавливать время хранения кэша и так далее. Советую со всем этим ознакомиться.

Начнем по порядку с кэширования на уровне всего сайта. Конечно, это может подойти только сайтам, которые практически не меняют своего содержимого и всем пользователям отдаются одинаковые страницы. Например, справочники, словари и прочее.

Для подключения такого кэширования, нам нужно в файле settings.py в коллекции MIDDLEWARE прописать два компонента, как показано ниже:
```python
# sitewomen/settings.py
MIDDLEWARE = [
    'debug_toolbar.middleware.DebugToolbarMiddleware',
...
```
Обратите внимание на порядок строк. Компоненты в списках файла settings.py выполняются в прописанном порядке. Поэтому важно их местоположение в коллекции MIDDLEWARE (DebugToolbar должен идти вначале).

Ниже в этом же файле следует прописать параметры:
```python
# sitewomen/settings.py
...
```
Значение 'default' определяет стандартные настройки работы сайтового кэша; значение 10 – задает время обновления кэша в секундах; префикс 'sitewomen' позволяет избегать коллизий, если одно и то же хранилище для кэша, например, Memcached или Redis используется для множества сайтов.

Запустим сервер Redis и наш тестовый веб-сервер. Переходим на главную страницу и видим, что при обновлении страниц время их загрузки сокращается, как раз, благодаря использованию сайтового кэша.

Уберем это кэширование и рассмотрим более практичный вариант - [«Кэширование на уровне представлений»](https://docs.djangoproject.com/en/4.2/topics/cache/#the-per-view-cache).

В документации приведен пример реализации кэша через декоратор cache_page, у которого в круглых скобках указывается время хранения кэша в секундах. Но, мы используем не функции, а классы представлений, поэтому кэширование будем делать на уровне URLconf. Опять же в документации представлен пример. Нам здесь вначале нужно импортировать декоратор cache_page, указать время хранения кэша и представление, связанное с конкретным URL. Давайте это сделаем. Откроем файл women/urls.py и вначале пропишем импорт:
```python
# women/views.py
...
...
```
Все, теперь в течение 30 секунд главная страница будет использовать кэш, то есть, любые изменения, которые будут вноситься в нее в течение этого времени, не будут отображаться в браузере. Это следует учитывать, используя кэширование.

Проверим, как это работает. Перейдем в браузер. Сейчас для формирования главной страницы используется 199 мс и шесть SQL-запросов. Обновим ее, все без изменений, так как сейчас было выполнено формирование кэша страницы. Обновим главную страницу еще раз и видим примерно 67 мс время формирования страницы и нулевое число SQL-запросов, то есть, был использован кэш. Через 30 секунд страница снова будет полностью строиться по алгоритму приложения, но затем, снова браться из кэша.
#### Кэширование на уровне шаблонов.
Мы увидели, как выполняется кэширование отдельных страниц на уровне представлений. Но так делают не часто. Иногда полезнее реализовать кэширование на уровне шаблонов. Например, мы можем выполнить кэширование отдельно сайдбара, а не всей страницы. Для этого откроем базовый шаблон base.html и вначале (или в любом удобном месте) импортируем специальный тег для реализации кэша:
```html
<!-- base.html -->
{% load cache %}
```
...
{% endcache %}
```
Если теперь обновить главную страницу, то увидим только четыре SQL-запроса для ее формирования.
...
Давайте воспользуемся этим функционалом и закэшируем список постов главной страницы. Откроем файл women/views.py, где сначала импортируем модуль:
```python
# women/views.py
from django.core.cache import cache
```
А, затем, в классе WomenHome  будем делать выборку постов из кэша с помощью функции get(): 
```python
# women/views.py
def get_queryset(self):
...
```
Он ничего не кэширует, но позволяет нам не запускать каждый раз при отладке Redis-сервер и, соответственно, видеть выполнение всех SQL-запросов.
### 81. Добавление карты сайта (sitemap).
Каждый состоятельный сайт почти всегда имеет [карту сайта](https://docs.djangoproject.com/en/4.2/ref/contrib/sitemaps/) – специальный файл, который содержит список актуальных URL-адресов, частоту их обновления, дату последнего обновления и так далее. Как правило, карта сайта представляется в формате XML и передается поисковым системам (Яндекс, Google) для лучшего ранжирования сайта в поисковой выдаче. Конечно же, во фреймворк Django встроена поддержка карты сайта и на этом занятии мы с вами научимся использовать этот инструмент.

Django содержит два специальных приложения:
- django.contrib.sites – управление глобальными настройками сайтов, с которыми может быть ассоциирован текущий проект;
- django.contrib.sitemaps – пакет для формирования карты сайта.

Вначале, конечно же, мы должны прописать эти приложения в коллекции INSTALLED_APPS файла settings.py:
```python
# sitewomen/settings.py
INSTALLED_APPS = [
    ...
...
```
И обязательно прописать параметр:
```python
# sitewomen/settings.py
SITE_ID = 1
```
который определяет id записи с настройками сайта, используемого модулем sitemaps.

После этого необходимо выполнить миграцию для создания новых таблиц:
```shell
python3 manage.py migrate
```
Если теперь зайти в админ-панель, то в разделе «Сайты» увидим запись с идентификатором 1 и доменным именем example.com. Именно этот номер идентификатора мы указали в параметре SITE_ID.  Таким образом, выбрав этот домен сайта. Конечно, его можно изменить, но сделаем это несколько позже, т.к. на работу сайта он никак влиять не будет.

Следующим шагом нам нужно объявить класс, который будет формировать данные для карты сайта. Для этого в приложении women создадим еще один файл с именем sitemaps.py и следующим содержимым:
```python
# women/sitemaps.py
...
```
Мы здесь наследуемся от специального базового класса Sitemap, затем, определяем два наиболее частых атрибута: changefreq – частота изменения постов; priority – приоритет постов. Подробную информацию о возможных атрибутах и их значениях можно посмотреть на странице [документации](https://docs.djangoproject.com/en/4.2/ref/contrib/sitemaps/#sitemap-class-reference).

Метод items() определяет набор записей, которые попадут в карту сайта (это все опубликованные посты). Метод lastmod() вызывается для каждой записи и через объект obj мы возвращаем дату последнего изменения поста. Атрибут time_update – это атрибут модели Women. Конкретные URL-адреса для каждой записи будут определяться путем вызова метода get_absolute_url(), определенного в модели Women.

Остался последний шаг – определить маршрут для карты сайта. Для этого откроем файл urls.py пакета конфигурации и добавим в него следующие строчки импорта:

Далее, определим специальный словарь:
```python
# sitewomen/urls.py
sitemaps = {
    'posts': PostSitemap,
...
```
который будем передавать в функцию представления sitemap. Ключ posts мы придумываем сами и можно указать любую другую строку. Функция sitemap автоматически создаст объект класса PostSitemap и на его основе сформирует элементы (набор ссылок) в карте сайта.

Наконец, в коллекции urlpatterns нужно прописать маршрут, следующим образом:
```python
# sitewomen/urls.py
urlpatterns = [
    ...
...
```
Мы указываем путь sitemap.xml, по которому будет доступна карта, затем, стандартную функцию представления с передачей ей требуемых данных. Имя маршрута определяем сами, часто прописывают путь к используемой функции представления.

Все, карта сайта добавлена на сайт. Если сейчас перейти по адресу:
```http request
http://127.0.0.1:8000/sitemap.xml
```
то должны увидеть XML-файл со ссылками на опубликованные посты. При этом домен будет браться из БД, прописанный в разделе «Сайты». Давайте поменяем его на локальный хост 127.0.0.1:8000. Обновляем карту и видим теперь другой домен. Вот так можно управлять доменным именем в карте сайта.

Если в эту же карту нам нужно добавить записи из еще какой-нибудь другой таблицы, например, Category, то для этого в файле women/sitemaps.py достаточно прописать еще один класс:
```python
# women/sitemaps.py
...
```
и указать его в словаре sitemaps в файле urls.py:
```python
# sitewomen/urls.py
sitemaps = {
    'posts': PostSitemap,
...
http://127.0.0.1:8000/sitemap.xml
```
и видим дополнительные URL-адреса.

И последний штрих. Обычно, карты сайта меняются крайне редко. Поэтому функцию представления sitemap целесообразно закешировать, например, так:
```python
# sitewomen/urls.py
...
...
...
```
Теперь, при частом обращении к этой странице сервер не будет сильно нагружаться. Хотя, маловероятно, что к этой странице кто то будет обращаться слишком часто. Как я отмечал в самом начале, она предназначена для сервисов поисковых систем, а не для пользователей. Но, тем не менее, злоумышленник может воспользоваться высокой нагрузкой формирования этой страницы и часто ее вызывать, чтобы «положить» сервер с сайтом. Кэширование здесь кардинально изменит ситуацию.

Вот так, в самом простом варианте можно создавать карты сайта с помощью фреймворка Django. С дополнительной информацией можно ознакомиться на странице [документации](https://docs.djangoproject.com/en/4.2/ref/contrib/sitemaps/).
## XII. Основы тестирования с unittest. 
### 82. Первый тест с Django unittest.
На данный момент у нас с вами получился некий аналог информационного сайта. И, вроде бы, он полностью функционален. Но так ли это на самом деле? И как можно быть более-менее уверенным, что мы все сделали правильно, и что он полностью готов к развертыванию на реальном веб-сервере? Для этого в программировании существует этап тестирования программного кода. Причем, тестирование выполняется и непосредственно в процессе написания программы, и после полного завершения работы над всем проектом. Но, что из себя представляет процесс тестирования? В действительности, это многоуровневая проверка, начиная с программного тестирования, и заканчивая пользовательским тестированием. Мы с вами затронем тестирование программного кода с помощью дополнительно написанных тестовых функций. Как раз этот процесс автоматизирует модуль [unittest](https://docs.python.org/3/library/unittest.html) языка Python.

Общая идея следующая. В файле tests.py, который есть в каждом приложении проекта Django, создается класс на базе класса TestCase, который будет содержать определенную группу тестов в виде набора функций с префиксом test. Например, вот так можно определить класс из набора тестов, покрывающих проверку доступа к различным страницам сайта:
```python
# women/tests.py
...
```
Базовый класс TestCase поддерживает специальные необязательные методы setUp() и tearDown(), которые вызываются до и после выполнения каждой тестовой функции. А сами тесты определены в виде методов с префиксом test. Число таких методов-тестов может быть произвольным (столько, сколько нужно).
#### Способы запуска тестов.
Предположим, что мы определили несколько тестирующих методов. Следующий вопрос, как их запускать? Один из вариантов – это командная строка терминала. Если записать:
```shell
python3 manage.py test .
```
то в текущем проекте будет выполнен поиск всех тестов во всех приложениях и последовательно запущены. Если нам нужно запустить тесты строго определенного приложения, то используется команда:
```shell
python3 manage.py test women
```
Соответственно в приложении women в файле tests.py будет выполнен поиск тестовых функций (методов) с их автоматическим запуском. Либо, можно отдельно указать класс (группу) тестов:
```shell
python3 manage.py test women.tests.GetPagesTestCase
```
И, наконец, запуск отдельного теста:
```shell
python3 manage.py test women.tests.GetPagesTestCase.test_case_1
```
Во всех случаях видим, что в терминале отображаются сообщения о создании БД с алиасом 'default' и ее последующего уничтожения. Это специальная временная БД, которая подменяет собой БД проекта, чтобы методы тестирования не вносили изменения в реальные данные сайта.

Другой способ запуска – непосредственно из среды PyCharm. Зайдем в редактирование конфигураций запуска и для sitewomen создадим копию. Назовем ее test_get_pages, а в параметрах вместо runsrver пропишем:

test women.tests.GetPagesTestCase

Все, теперь мы можем достаточно быстро запускать группу тестов класса GetPagesTestCase, в том числе и в режиме отладки.
#### Создание простых тестов.
Следующим шагом мы с вами посмотрим, как можно создавать простые тесты. Так как наш класс GetPagesTestCase отвечает за проверку получения страниц сайта, то и тесты в нем следует прописывать исключительно для этих целей. Первым делом посмотрим, возвращается ли главная страница:
```python
# women/tests.py
class GetPagesTestCase(TestCase):
    def test_mainpage(self):
        ...
        print(response)
```
Мы здесь с помощью стандартной функции reverse вычисляем URL-адрес главной страницы,  а затем, обращаемся к специальному объекту client. Он позволяет нам имитировать различные запросы к серверу (как будто они идут от браузера) и получать результат ответа сервера. Это переменная response. В данном случае нам нужно сымитировать  GET-запрос для получения главной страницы сайта. Поэтому у объекта client используется метод get() с указанием URL-адреса страницы. На выходе получаем объект ответа response, который отображаем в консоли.

Поставим точку останова на функции print, запустим тест в режиме отладки, и посмотрим на содержимое объекта response. Видим множество различных атрибутов, в том числе и атрибуты:
- content – содержимое HTML-страницы;
- context_data – словарь с переданными данными в шаблон;
- request – стандартный объект запроса;
- status_code – код статуса страницы;
- template_name – список используемых шаблонов.

и некоторые другие. Все эти значения можно использовать при тестировании.

Что именно тестировать и как писать тесты, мы решаем сами. В программировании это целое отдельное направление, которое посвящено правильному созданию тестов. А тех, кто их пишут, называют тестировщиками. Разумеется, тесты должны покрывать все возможные критические варианты исполнения кода программы. Я лишь в учебных целях приведу пример того, как это можно делать для проверки работоспособности сайта.

Давайте, первым делом убедимся, что страница успешно возвращается клиенту, то есть, мы должны иметь значение параметра status_code 200. Для этого воспользуемся специальным методом assertEqual(), который выполняет проверку на равенство двух переданных аргументов:
```python
# women/tests.py
...
    def test_mainpage(self):
...
...
```
Если аргументы равны, то тест проходит, иначе, генерируется исключение и тест завершается с ошибкой.

Сейчас тест проходит, но если, например, вместо кода 200 записать любое другое значение, скажем, 302, то увидим сообщение:

FAILED (failures=1)

и строчку, в которой было найдено несоответствие. Вот пример того, как формируются и запускаются тесты. Подробную информацию о различных assert-методах можно посмотреть на странице [документации](https://docs.python.org/3/library/unittest.html) unittest.

Дополнительно в этом же тесте я пропишу проверку на соответствие шаблона главной страницы и заголовка (title):
```python
# women/tests.py
class GetPagesTestCase(TestCase):
    def test_mainpage(self):
...
```
Обратите внимание на метод assertIn(). Он проверяет вхождение значения в коллекцию данных. Дело в том, что template_name представляет собой список из набора наследуемых и расширяемых шаблонов и нам нужно убедиться, что среди них есть шаблон с именем 'women/index.html'. Проверку на равенство здесь уже делать не нужно. А последний элемент мы снова проверяем с помощью метода assertEqual(), используя данные из словаря context_data.

Однако для проверки использования того или иного шаблона есть свой отдельный метод assertTemplateUsed(), которым мы и воспользуемся:
```python
# women/tests.py
self.assertTemplateUsed(response, 'women/index.html')
```
Последний штрих, который здесь следует сделать – это вместо конкретного числа 200 (или любого другого) воспользуемся предопределенными значениями о HTTP-статусах. Импортируем из http модуль HTTPStatus:
```python
# women/tests.py
from http import HTTPStatus
```
и из него выберем статус с именем OK:
```python
# women/tests.py
self.assertEqual(response.status_code, HTTPStatus.OK)
```
Давайте в этот же класс добавим еще один тест, который будет проверять перенаправление со страницы:
```http request
http://127.0.0.1:8000/addpage/
```
на страницу авторизации для неавторизованных пользователей. Для этого запишем еще один метод в класс GetPagesTestCase следующим образом:
```python
# women/tests.py
...
...
...
```
Вначале создаются вспомогательные переменные path и redirect_uri, а затем, выполняется проверка кода статуса 302 страницы addpage и перенаправление на страницу с URL-адресом redirect_uri. Для этого используется еще один assert-метод assertRedirects().

Вот общий принцип построения простых тестов для проверки работоспособности проекта сайта на фреймворке Django.
### 83. Использование БД в unittest фреймворка Django.
На прошлом занятии мы с вами написали простые тесты для проверки корректности ответа сервера двух страниц нашего сайта. Но этого явно недостаточно. Необходимо также проверять содержимое страниц, а для этого нам нужны записи из БД.

На прошлом занятии я отмечал, что при запуске каждого теста создается вспомогательная БД с набором пустых таблиц нашего проекта и закрывается после прохождения очередного теста. В этом легко убедиться. Пропишем еще один тест со следующим содержимым: 
```python
# women/tests.py
...
...
...
```
Запустим его в режиме отладки и видим пустой список w и пустой список коллекции response.context_data[‘posts’]. Это значит, что тестовая БД не содержит ни одной записи. И когда мы делаем тестовый GET-запрос, то также шаблоны страницы не наполняются никаким содержимым. Как, спрашивается, тогда сформировать данные? Для этого базовый класс TestCase поддерживает специальный атрибут fixtures, который может содержать список из json-файлов с данными, которые автоматически загружаются в БД при запуске каждого теста.

Создавать json-файлы с данными мы с вами уже умеем. Это можно сделать следующими командами:
- python3 manage.py dumpdata women.women -o women/fixtures/women_women.json
- python3 manage.py dumpdata women.category -o women/fixtures/women_category.json
- python3 manage.py dumpdata women.husbands -o women/fixtures/women_husbands.json
- python3 manage.py dumpdata women.tagspost -o women/fixtures/women_tagspost.json

А, затем, указать их в параметре fixtures:
```python
# women/tests.py
class GetPagesTestCase(TestCase):
    ...
    ...
```
Обратите внимание, мы должны прописать все четыре файла, так как модель Women имеет внешние ключи, на остальные три таблицы БД. Если убрать хотя бы одну фикстуру, то получим ошибку загрузки данных в БД.

Теперь, при запуске последнего теста, мы с вами увидим содержимое таблицы Women. Осталось проверить, совпадает ли оно с данными, которые передаются на главную страницу. Для этого воспользуемся еще одним методом assertQuerysetEqual() следующим образом: 
```python
# women/tests.py
...
    def test_data_mainpage(self):
...
Если попробовать выполнить этот тест, то видим, что он успешно проходит. Если же вместо пяти указать любое другое значение, то получим ошибку несоответствия данных.

Вас может несколько обескуражить, что при выполнении каждого теста снова и снова создается БД с набором таблиц и загрузкой записей из указанных фикстур. Это довольно ресурсоемкая операция. Но тесты не предназначены для быстрого выполнения. У них другая роль – проверить корректность формирования ответа от сервера. И важно лишь, чтобы они покрывали все возможные критические исходы и фиксировали несоответствия. Все остальное, включая и некоторое дублирование кода в каждом тестовом методе – совершенно не критично. Благодаря такому изолированному выполнению тестов, результат работы одного, никак не скажется на результатах другого. Именно из этих соображений БД каждый раз создается заново при запуске очередного теста.

Давайте добавим еще один тест, который будет проверять работу пагинации главной страницы:
```python
# women/tests.py
...
...
...
```
Мы здесь через параметр page дополнительно проверяем корректность выдачи списка для указанной страницы. При запуске видим, что у нас четыре теста и все они проходят успешно.

В заключение занятия сделаем проверку содержимого страницы отображения поста. Сформируем еще один тест следующим образом:
```python
# women/tests.py
...
