...
register_converter(FourDigitYearConverter, 'year4')  # Регистрируем созданный конвертер. Первый аргумент - сам конвертер,
# второй аргумент - имя конвертера для urlpatterns.

urlpatterns = [  # В этой коллекции можно прописать сколько угодно маршрутов для страниц отображения клиенту.
    path('', index),
    path('cats/<int:cat_id>/', categories),  # По стандартам отрасли следует прописывать слэш в конце суффикса,
    # ибо если этого не сделать, то вместо привычного 127.0.0.1:8000/cats/ нужно будет писать 127.0.0.1:8000/cats, что неудобно.
    path('cats/<slug:cat_slug>/', categories_by_slug),  # Django проверяет маршруты в порядке их записи, так что
    # int следует ставить раньше, нежели slug, иначе отработает slug.
